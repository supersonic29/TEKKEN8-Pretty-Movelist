		...
		d3.json("./assets/data/map_ctrls.json", function (err, data) {
			...
			d3.json("./assets/data/map_chars.json", function (err, data) {
				...
				for (let i = 0; i < data.length; i++) {
					var characterName = data[i].c_index.split(" ");
					const isJack = () => data[i].i == "11"
					if (isJack) characterName = data[i].c.split("-")
					generateCharThumbnails(data, i, characterName)
					addLoadMovesOnClick(data, i)
				}
				var id_string = char_data[selected_char].c.split(" ");
				d3.select("#" + id_string[0]).classed("selected", true);
				d3.select("#selected-title").text(char_data[selected_char].n);
...
const generateCharThumbnails = (data, i, characterName) => {
	if (characterName[0] !== 'PAUL') return
	d3.select(".char-menu > .inner-table > table").append("tr")
		.html("<td class=\"char-card\" id=\"" + data[i].c.split(" ")[0] + "\"><img src=\"./assets/chars/" + characterName.join("").toLowerCase() + "_thumbnail.png\"><p>" + data[i].c + "</p></td>");
}
const addLoadMovesOnClick = (data, i) => {
			...
			commands.forEach(command => {
				...
				if (/[a-z]/.test(command.toLowerCase()))
					...
					for (let m = 0; m < command.length; m++) {
						try {
							if (isLetter(ctrls_map[command.charAt(m)])) {
								if (ctrls_map[command.charAt(m)] === ctrls_map[command.charAt(m)].toLowerCase() || ctrls_map[command.charAt(m)] === "N")
									html_string += "<img class=\"move-arrow\" src=\"./assets/arrow/" + ctrls_map[command.charAt(m)].toLowerCase() + ".svg\">";
								else html_string += "<img class=\"move-arrow\" src=\"./assets/arrow/" + ctrls_map[commancommand.charAt(m)].toLowerCase() + "p" + ".svg\">";
							}
							else if (!isNaN(ctrls_map[command.charAt(m)].charAt(0)))
								html_string += "<img class=\"move-button\" src=\"./assets/button/" + button_layouts[bl_choice] + "/" + ctrls_map[command.charAt(m)] + ".svg\">";
							else if (ctrls_map[command.charAt(m)] === ">") {
								...
								console.log("2. Not added map: " + ctrls_map[command.charAt(m)]);
							}
						} catch (e) {
